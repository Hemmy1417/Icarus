/**
 * Deploy ICARUS to GenLayer Studio Next and verify the bytes.
 *
 *   node scripts/deploy.mjs [label]            deploy contracts/icarus.py, record it under label
...
const SOURCE = fileURLToPath(new URL("../contracts/icarus.py", import.meta.url));
...
async function deploy(label) {
  ...
  console.log(`deploying contracts/icarus.py (sha256 ${sha(code)}) on chain ${chain.id}`);
...
async function verify(address) {
  ...
  if (live !== repo) {
    ...
    console.error("verify: the deployed source does NOT match contracts/icarus.py");
    process.exit(1);
  }
